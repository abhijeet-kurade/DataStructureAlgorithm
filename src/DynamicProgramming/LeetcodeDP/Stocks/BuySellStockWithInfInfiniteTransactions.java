...
public class BuySellStockWithInfInfiniteTransactions {
    public static void main(String[] args) {
        int[] prices = {7, 1, 6, 3, 4, 5};
        System.out.println(new Memoization().maxProfit(prices));
    }
    ...
    private static class Recursive{
        ...
        public static int maxProfit(int[] prices, int idx, boolean isBuy){
            ...
            if(isBuy){
                ...
                int sell = prices[idx] + maxProfit(prices, idx+1, !isBuy);
                int skip = maxProfit(prices, idx+1, isBuy);
    ...
    private static class Memoization {
        ...
        public static int maxProfit(int[] prices, int idx, int isBuy, int[][] dp){
            ...
            if(isBuy == 0){
                ...
                int sell = prices[idx] + maxProfit(prices, idx+1, isBuy^1, dp);
                int skip = maxProfit(prices, idx+1, isBuy, dp);
                profit = Math.max(sell, skip);
            }
            dp[idx][isBuy] = profit;
